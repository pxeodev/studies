...
const ConnectButton = ({ collapsed }) => {
  ...
  const openModalOrDisconnect = useCallback(() => {
    if (nativeWalletAddress) {
      ...
      // TODO: Telegram disconnect, if tg bot is connected
      setLoginModalVisible(true)
    }
  }, [nativeWalletAddress, nativeDisconnect])
  ...
  const telegramConnectOrDisconnect = useCallback(() => {
    // TODO: Telegram disconnect
    if (nativeWalletAddress) {
      nativeDisconnect()
    } else {
      // TODO: Correct tg bot link
      window.open('https://t.me/CRtesting_bot','_blank');
    }
  }, [nativeWalletAddress, nativeDisconnect])
  ...
  if (collapsed) {
    if (nativeWalletAddress) {
      text = `0x${nativeWalletAddress.slice(2, 4).toUpperCase()}...${nativeWalletAddress.slice(-4).toUpperCase()}`
    }
  } else {
    if (nativeWalletAddress) {
      text = `0x${nativeWalletAddress.slice(2, 8).toUpperCase()}...${nativeWalletAddress.slice(-8).toUpperCase()}`
    } else {
      text = 'Connect'
    }
  }
  ...
  return (
    <>
      <Button
        ...
        className={classnames(connectButtonStyles.button, { [connectButtonStyles.connected]: Boolean(nativeWalletAddress), [connectButtonStyles.collapsed]: collapsed })}
      >
        <span className={connectButtonStyles.text}>{text}</span>
        {nativeWalletAddress ? <></> : <span className={connectButtonStyles.iconWrapper}>
