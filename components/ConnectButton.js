...
const ConnectButton = () => {
  ...
  if (walletAddress) {
    shortenedWalletAddress = `0x${walletAddress.slice(2, 8).toUpperCase()}...${walletAddress.slice(-8).toUpperCase()}`
  }
  ...
  return (
    <Button
      onClick={connect}
      disabled={disabled}
      className={classnames(connectButtonStyles.button, { [connectButtonStyles.connected]: Boolean(walletAddress) })}
    >
      {walletAddress ? shortenedWalletAddress : <span className={connectButtonStyles.text}>Connect Wallet</span>}
