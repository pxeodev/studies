...
export const getOpenInterest = async (symbol, market, openInterestDenominationPriceFactor) => {
  const data = await coinalyze.get(`open-interest?symbols=${symbol}`)
  const value = data.data[0]?.value * openInterestDenominationPriceFactor
  return { openInterest: value, symbol, market }
}

export const getFundingRate = async (symbol, market) => {
  const data = await coinalyze.get(`predicted-funding-rate?symbols=${symbol}`)
  const value = data.data[0]?.value
  return { fundingRate: value, symbol, market }
}

export const getVolume24h = async (symbol, market, coinPrice) => {
  let yesterday = subDays(new Date(), 1)
  yesterday = parseInt(+yesterday / 1000)
  const today = parseInt(+new Date() / 1000)
  const data = await coinalyze.get(`ohlcv-history?symbols=${symbol}&interval=5min&from=${yesterday}&to=${today}`)
  const value = sum(data.data[0]?.history?.map(ohlc => ohlc.v))
  return { futuresVolume24h: value * coinPrice, symbol, market }
}
